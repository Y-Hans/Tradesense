...
void main() {
  group('ConnectivityBanner Widget Tests', () {
    ...
    testWidgets(
        'unknown status -> banner is hidden (0 height, no layout space)',
      ...
      expect(find.text("Some live features are temporarily unavailable."),
          findsNothing);
    ...
    testWidgets(
        'offline status -> persistent banner appears with title, subtitle, and icon',
        (WidgetTester tester) async {
      fakeService = _FakeConnectivityService(
    ...
    testWidgets(
        'global integration: AppShell & GlobalStatusRegion display banner globally',
        (WidgetTester tester) async {
      fakeService = _FakeConnectivityService(
        initialStatus: ConnectivityStatus.offline,
      );
    ...
    testWidgets(
        'GlobalStatusRegion preserves intended banner ordering in layout hierarchy',
        (WidgetTester tester) async {
      fakeService = _FakeConnectivityService(
        initialStatus: ConnectivityStatus.offline,
      );
      ...
      final Column columnWidget = tester.widget<Column>(
        find.descendant(
          ...
          matching: find.byWidgetPredicate(
            (w) =>
                w is Column && w.children.any((c) => c is ConnectivityBanner),
          ),
        ),
      );
