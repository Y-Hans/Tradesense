...
class FakeMarketProvider implements MarketProvider {
  ...
  Future<List<CryptoAsset>> getSupportedAssets() async {
    ...
    if (shouldThrow) {
      throw customError ?? Exception('Network connection failed');
    }
...
void main() {
  group('MarketCacheKeys Tests', () {
    test('Formats key strings consistently and normalizes symbols to uppercase',
        () {
      expect(
          MarketCacheKeys.supportedAssets, equals('market_supported_assets'));
  ...
  group('CachedMarketRepository Unit Tests', () {
    ...
    test('Cache Miss: Fetches from API and populates cache', () async {
      ...
      final exists =
          await assetCache.containsKey(MarketCacheKeys.supportedAssets);
      expect(exists, isTrue);
    });

    test('Cache Hit: Subsequent read returns cached data without calling API',
        () async {
      // First call (miss)
    ...
    test('Expired Cache: Advance time beyond TTL triggers fresh API fetch',
        () async {
      // Warm up cache
      await cachedRepo.getSupportedAssets();
      expect(fakeInner.getSupportedAssetsCallCount, equals(1));
    ...
    test(
        'Stale Cache Fallback: Returns stale cached data when API fails and allowStale is true',
        () async {
      // Warm up cache at t0
      await cachedRepo.getSupportedAssets();
      expect(fakeInner.getSupportedAssetsCallCount, equals(1));
    ...
    test(
        'API Failure Without Cache: Rethrows exception when no cache is available',
        () async {
      fakeInner.shouldThrow = true;
    ...
    test(
        'Cache Version Mismatch: Schema evolution invalidates old version and refreshes from API',
        () async {
    ...
    test(
        'Cache Version Mismatch with API Failure: Falls back to stale entry when allowStale is true',
        () async {
      // Seed cache directly with old version 'v0'
      const oldPolicy = CachePolicy(ttl: Duration(minutes: 60), version: 'v0');
      await assetCache.set(
    ...
    test(
        'Stream Ticker: Delegates directly to inner provider without cache side-effects',
        () async {
      ...
      final hasKey =
          await tickerCache.containsKey(MarketCacheKeys.ticker('BTC'));
      expect(hasKey, isFalse);
    });

    test(
        'Ticker & Candle Cache operations operate correctly through generic repository',
        () async {
