...
void main() {
  group('DefaultCacheRepository Unit Tests', () {
    ...
    test('deterministic expiration with injected clock', () async {
      ...
      expect((resultExpired as CacheExpired<String>).staleEntry?.value,
          equals('v1'));
    });

    test(
        'expired entry returns CacheHit with isStale=true when allowStale is true',
        () async {
      ...
      const getPolicyWithStale =
          CachePolicy(ttl: Duration(minutes: 10), allowStale: true);
    ...
    test('corrupted data returns CacheFailure gracefully without crashing',
        () async {
