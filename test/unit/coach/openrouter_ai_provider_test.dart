...
void main() {
  group('OpenRouterAIProvider Unit Tests', () {
    ...
    test('Successful 200 response parses into valid CoachResponse', () async {
      final validFeedback = {
        ...
        'what_to_consider_next':
            'Diversify across multiple non-correlated assets.',
      };
    ...
    test(
        'Empty API key throws OpenRouterException before making network request',
        () async {
    ...
    test(
        'Malformed response with empty feedback fields throws OpenRouterValidationException',
        () async {
    ...
    test('Non-JSON completion message content throws OpenRouterFormatException',
        () async {
      mockAdapter.responseHandler = (_) => {
            'choices': [
              {
    ...
    test(
        'HTTP 429 Rate Limit error throws OpenRouterException with status code',
        () async {
