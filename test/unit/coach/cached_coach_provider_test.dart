...
void main() {
  ...
  group('CachedCoachProvider Unit Tests', () {
    test('Cache Miss: fetches fresh response from inner provider and caches it', () async {
      final response = await cachedCoachProvider.generateCoachFeedback(sampleRequest);

      expect(response.whatDoneWell, equals('Executed order with defined boundaries.'));
      expect(fakeInnerProvider.generateCallCount, equals(1));

      // Subsequent call should hit cache and NOT invoke inner provider again
      final secondResponse = await cachedCoachProvider.generateCoachFeedback(sampleRequest);
      expect(secondResponse.whatDoneWell, equals('Executed order with defined boundaries.'));
      expect(fakeInnerProvider.generateCallCount, equals(1)); // Still 1
    });

    test('Cache Hit: returns cached data immediately without calling inner provider', () async {
      // Warm up cache
      await cachedCoachProvider.generateCoachFeedback(sampleRequest);
      expect(fakeInnerProvider.generateCallCount, equals(1));

      // Execute hit call
      final cachedResponse = await cachedCoachProvider.generateCoachFeedback(sampleRequest);
      expect(cachedResponse.whatDoneWell, equals(fakeInnerProvider.responseToReturn.whatDoneWell));
      expect(fakeInnerProvider.generateCallCount, equals(1));
    });

    test('Expired Cache: attempts fresh generation and updates cache', () async {
      // 1. Initial write
      await cachedCoachProvider.generateCoachFeedback(sampleRequest);
      expect(fakeInnerProvider.generateCallCount, equals(1));
      ...
      final freshResponse = await cachedCoachProvider.generateCoachFeedback(sampleRequest);

      expect(freshResponse.whatDoneWell, equals('Refreshed coaching after TTL expiration.'));
      expect(fakeInnerProvider.generateCallCount, equals(2));
    });

    test('Stale Cache Fallback: returns stale cached response when fresh generation fails and allowStale is true', () async {
      // 1. Initial write
      await cachedCoachProvider.generateCoachFeedback(sampleRequest);
      ...
      final fallbackResponse = await cachedCoachProvider.generateCoachFeedback(
        sampleRequest,
        policy: CachePolicy.withTtl(const Duration(hours: 24), allowStale: true),
      );

      expect(fallbackResponse.whatDoneWell, equals('Executed order with defined boundaries.'));
      expect(fakeInnerProvider.generateCallCount, equals(2));
    });

    test('Generation Failure without Cache: rethrows exception when no cache entry exists', () async {
      fakeInnerProvider.shouldThrow = true;
