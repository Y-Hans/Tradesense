...
class _FakeConnectivityService implements ConnectivityService {
  ...
  _FakeConnectivityService({ConnectivityStatus initialStatus = ConnectivityStatus.online})
...
void main() {
  ...
  group('ConnectivityNotifier & Riverpod Provider Unit Tests', () {
    ...
    test('ConnectivityNotifier defaults to unknown state before async check resolves', () {
      final notifier = ConnectivityNotifier(fakeService);
      expect(notifier.state, equals(ConnectivityStatus.unknown));
      notifier.dispose();
    });

    test('ConnectivityNotifier initializes with service current status after async check', () async {
      fakeService = _FakeConnectivityService(
    ...
    test('ConnectivityNotifier updates state when service notifies onStatusChanged', () async {
      final notifier = ConnectivityNotifier(fakeService);
      await pumpEventQueue();
    ...
    test('connectivityProvider starts at unknown state synchronously then updates', () async {
      ...
      expect(container.read(connectivityProvider), equals(ConnectivityStatus.unknown));
    ...
    test('connectivityProvider notifies listeners on state change', () async {
      ...
      expect(listenedStates, equals([
        ConnectivityStatus.online, // transition from initial unknown -> resolved online
