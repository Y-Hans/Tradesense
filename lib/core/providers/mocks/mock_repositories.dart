...
class MockTradingRepository implements TradingRepository {
  ...
  double get totalPortfolioValueInr {
    final holdingsValue =
        _holdings.fold(0.0, (sum, h) => sum + (h.quantity * h.currentPriceInr));
    return _wallet.balanceInr + holdingsValue;
  }

  void updateWallet(VirtualWallet updatedWallet) {
    _wallet = updatedWallet;
  }

  void updateHolding(Holding updatedHolding) {
    final existingIndex = _holdings.indexWhere((h) => h.symbol == updatedHolding.symbol);
    if (existingIndex >= 0) {
      _holdings[existingIndex] = updatedHolding;
    } else {
  ...
  void _subscribeToTickerIfNeeded(String symbol) {
    if (!_tickerSubscriptions.containsKey(symbol)) {
      _tickerSubscriptions[symbol] =
          _marketProvider.streamTicker(symbol).listen((ticker) {
  ...
  void _onTickerUpdate(MarketTicker ticker) {
    ...
    if (index >= 0) {
      _holdings[index] =
          _holdings[index].copyWith(currentPriceInr: ticker.priceInr);
    }

    // Evaluate stop losses
    final triggered = _activeStopLosses
        .where((sl) =>
            sl.symbol == symbol && ticker.priceInr <= sl.stopLossPriceInr)
        .toList();
  ...
  Future<Trade> executeMarketBuy({
    ...
    if (stopLossPriceInr != null && stopLossPriceInr >= executionPriceInr) {
      throw Exception(
          'Stop-loss price must be below the current market price for buy orders.');
    }
    ...
    if (totalCost > maxAllowedSize) {
      throw Exception(
          'Position size exceeds the 25% limit (Max allowed: $maxAllowedSize).');
    }
    ...
    final trade = Trade(
      ...
      riskScoreAtTrade:
          (totalCost / portfolioValue * 100.0).round().clamp(10, 90),
    );
  ...
  Future<Trade> executeMarketSell({
    ...
    if (existingIndex < 0 || _holdings[existingIndex].quantity < quantity) {
      throw Exception(
          'Insufficient holding quantity for $symbol. Cannot oversell.');
    }
